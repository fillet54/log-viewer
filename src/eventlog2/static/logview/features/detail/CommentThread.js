(function () {
  ...
  const CommentThread = ({ threads, depth = 0, onReply }) => {
    ...
    const CommentThreadComponent = ui.components.CommentThread;
          ...
          (comment) => html`
            <div class="comment-item" data-comment-id=${comment.id} style=${{ marginLeft: `${depth * 16}px` }}>
              ...
              ${comment.replies?.length
                ? html`<${CommentThreadComponent} threads=${comment.replies} depth=${depth + 1} onReply=${onReply} />`
                : null}
            </div>
          `
        )}
  ...
  ui.components.CommentThread = CommentThread;
})();
